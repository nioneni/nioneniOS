...
{
    setupUi(this);
    labelMX->setPixmap(QPixmap("/usr/share/nioneniOS-installer/nioneniOS-globe-3.png"));
    char line[260];
    char *tok;
    FILE *fp;
    int i;
    ...
    // locale
    localeCombo->clear();
    fp = popen("cat /usr/share/nioneniOS/locales.template", "r");
    if (fp != NULL) {
        while (fgets(line, sizeof line, fp) != NULL) {
            i = strlen(line);
            line[--i] = '\0';
...
// unmount nioneniOS in case retrying
void MInstall::prepareToInstall()
{
    updateStatus(tr("Ready to install nioneniOS Linux filesystem"), 0);

    // unmount /boot/efi if mounted by previous run
    if (system("mountpoint -q /mnt/nioneniOS/boot/efi") == 0) {
        system("umount /mnt/nioneniOS/boot/efi");
    }

    // unmount /home if it exists
    system("/bin/umount -l /mnt/nioneniOS/home >/dev/null 2>&1");
    system("/bin/umount -l /mnt/nioneniOS >/dev/null 2>&1");
...
{
    QString cmd = QString("/sbin/mkswap %1 -L nioneniOSswap").arg(dev);
    if (system(cmd.toUtf8()) != 0) {
        // error
        return false;
    }
...
{    
    ...
    QString drv = QString("/dev/%1").arg(diskCombo->currentText().section(" ", 0, 0));
    QString msg = QString(tr("OK to format and use the entire disk (%1) for nioeniOS Linux?")).arg(drv);
    ...
    system("sleep 1");
    // mount partitions
    if (!mountPartition(rootdev, "/mnt/nioneniOS", mntops)) {
        return false;
    }

    // on root, make sure it exists
    system("sleep 1");
    mkdir("/mnt/nioneniOS/home",0755);
...
bool MInstall::makeChosenPartitions()
{
    ...
    if (saveHomeCheck->isChecked()) {
        ...
        if (homedev.compare("/dev/root") != 0) {
            // not on root
            // system("rm -r -d /mnt/nioneniOS/home >/dev/null 2>&1"); ///not sure why this was here
            updateStatus(tr("Mounting the /home partition"), 8);
            if (!mountPartition(homedev, "/mnt/nioneniOS/home", mntops)) {
                return false;
            }
        } else {
            // on root, make sure it exists
        ...
        // don't save home
        system("/bin/rm -r /mnt/nioneniOS/home >/dev/null 2>&1");
        mkdir("/mnt/nioneniOS/home",0755);
        if (homedev.compare("/dev/root") != 0) {
            // not on root
...
void MInstall::installLinux()
{
    ...
    if (isRootFormatted) {
        ...
        mountPartition(rootdev, "/mnt/nioneniOS", mntops); 
        ...
        QString cmd = QString("/bin/bash -c \"find /mnt/nioneniOS -mindepth 1 -maxdepth 1 ! -name home -exec rm -r {} \\;\"");
        proc->start(cmd);
    }
}
...
void MInstall::copyLinux()
{
    ...
    mkdir("/mnt/nioneniOS/opt", 0755);
    mkdir("/mnt/nioneniOS/dev", 0755);
    mkdir("/mnt/nioneniOS/proc", 0755);
    mkdir("/mnt/nioneniOS/sys", 0755);
    mkdir("/mnt/nioneniOS/run", 0755);
...
{
    QString cmd;
    QString val = getCmdOut("ls /mnt/nioneniOS/boot | grep 'initrd.img-3.6'");

    // the old initrd is not valid for this hardware
    if (!val.isEmpty()) {
        cmd = QString("rm -f /mnt/nioneniOS/boot/%1").arg(val);
        system(cmd.toUtf8());
    }
    ...
    system("mount -o bind /dev /mnt/nioneniOS/dev");
    system("mount -o bind /sys /mnt/nioneniOS/sys");
    system("mount -o bind /proc /mnt/nioneniOS/proc");

    // install new Grub now
    if (!grubEspButton->isChecked()) {
        cmd = QString("grub-install --target=i386-pc --recheck --no-floppy --force --boot-directory=/mnt/nioneniOS/boot /dev/%1").arg(boot);
    } else {
        system("mkdir /mnt/nioneniOS/boot/efi");
        QString mount = QString("mount /dev/%1 /mnt/nioneniOS/boot/efi").arg(boot);
        runCmd(mount);
        QString arch = getCmdOut("uname -m");
        if (arch == "i686") { // rename arch to match grub-install target
            arch = "i386";
        }
        cmd = QString("chroot /mnt/nioneniOS grub-install --target=%1-efi --efi-directory=/boot/efi --bootloader-id=nioneniOS16 --recheck").arg(arch);
    }
    if (runCmd(cmd) != 0) {
        // error, try again
        ...
        if (runCmd(cmd) != 0) {
            ...
            system("umount /mnt/nioneniOS/proc; umount /mnt/nioneniOS/sys; umount /mnt/nioneniOS/dev");
            if (system("mountpoint -q /mnt/nioneniOS/boot/efi") == 0) {
                system("umount /mnt/nioneniOS/boot/efi");
            }
            return false;
    ...
    cmd = QString("sed -i -r 's|^(GRUB_CMDLINE_LINUX_DEFAULT=).*|\\1\"%1\"|' /mnt/nioneniOS/etc/default/grub").arg(cmdline);
    system(cmd.toUtf8());
    // update grub config
    runCmd("chroot /mnt/nioneniOS update-grub");
    if (!isFormatBtrfsZlib && !isFormatBtrfsLzo) {
      runCmd("/sbin/make-fstab --install /mnt/nioneniOS --mntpnt=/media"); 
     } else {
      runCmd("/sbin/make-fstab -f /mnt/nioneniOS/etc/fstab --mntpnt=/media");
     }
	runCmd("chroot /mnt/nioneniOS dev2uuid_fstab");  
    runCmd("chroot /mnt/nioneniOS update-initramfs -u -t -k all");
    system("umount /mnt/nioneniOS/proc; umount /mnt/nioneniOS/sys; umount /mnt/nioneniOS/dev");
    if (system("mountpoint -q /mnt/nioneniOS/boot/efi") == 0) {
        system("umount /mnt/nioneniOS/boot/efi");
    }
...
bool MInstall::setUserName()
{
    ...
    QString dpath = QString("/mnt/nioneniOS/home/%1").arg(userNameEdit->text());
    ...
    if ((dir = opendir(dpath.toUtf8())) == NULL) {
        ...
        // copy skel to demo
        if (runCmd("cp -a /mnt/nioneniOS/etc/skel /mnt/nioneniOS/home") != 0) {
            setCursor(QCursor(Qt::ArrowCursor));
            QMessageBox::critical(0, QString::null,
                                  tr("Sorry, failed to create user directory."));
            return false;
        }
        cmd = QString("mv -f /mnt/nioneniOS/home/skel %1").arg(dpath);
        ...
        cmd = QString("cp -n /mnt/nioneniOS/etc/skel/.bash_profile %1").arg(dpath);
        runCmd(cmd.toUtf8());
        cmd = QString("cp -n /mnt/nioneniOS/etc/skel/.bashrc %1").arg(dpath);
        runCmd(cmd.toUtf8());
        cmd = QString("cp -n /mnt/nioneniOS/etc/skel/.gtkrc %1").arg(dpath);
        runCmd(cmd.toUtf8());
        cmd = QString("cp -n /mnt/nioneniOS/etc/skel/.gtkrc-2.0 %1").arg(dpath);
        runCmd(cmd.toUtf8());
        cmd = QString("cp -Rn /mnt/nioneniOS/etc/skel/.config %1").arg(dpath);
        runCmd(cmd.toUtf8());
        cmd = QString("cp -Rn /mnt/nioneniOS/etc/skel/.local %1").arg(dpath);
        runCmd(cmd.toUtf8());
    }
    ...
    // change in files
    replaceStringInFile("demo", userNameEdit->text(), "/mnt/nioneniOS/etc/group");
    replaceStringInFile("demo", userNameEdit->text(), "/mnt/nioneniOS/etc/gshadow");
    replaceStringInFile("demo", userNameEdit->text(), "/mnt/nioneniOS/etc/passwd");
    replaceStringInFile("demo", userNameEdit->text(), "/mnt/nioneniOS/etc/shadow");
    replaceStringInFile("demo", userNameEdit->text(), "/mnt/nioneniOS/etc/slim.conf");
    replaceStringInFile("demo", userNameEdit->text(), "/mnt/nioneniOS/etc/lightdm/lightdm.conf");
    if (autologinCheckBox->isChecked()) {
        replaceStringInFile("#auto_login", "auto_login", "/mnt/nioneniOS/etc/slim.conf");
        replaceStringInFile("#default_user ", "default_user ", "/mnt/nioneniOS/etc/slim.conf");
    }
    else {
        replaceStringInFile("auto_login", "#auto_login", "/mnt/nioneniOS/etc/slim.conf");
        replaceStringInFile("default_user ", "#default_user ", "/mnt/nioneniOS/etc/slim.conf");
        replaceStringInFile("autologin-user=", "#autologin-user=", "/mnt/nioneniOS/etc/lightdm/lightdm.conf");
    }
    cmd = QString("touch /mnt/nioneniOS/var/mail/%1").arg(userNameEdit->text());
    system(cmd.toUtf8());
    setCursor(QCursor(Qt::ArrowCursor));
    return true;
}
...
bool MInstall::setPasswords()
{
    setCursor(QCursor(Qt::WaitCursor));
    qApp->processEvents();
    FILE *fp = popen("chroot /mnt/nioneniOS passwd root", "w");
    bool fpok = true;
    ...
    fp = popen("chroot /mnt/nioneniOS passwd demo", "w");
    fpok = true;
...
bool MInstall::setComputerName()
{
    ...
    if (val.compare("installed") == 0) {
        ...
        replaceStringInFile("antix1", computerNameEdit->text(), "/mnt/nioneniOS/etc/samba/smb.conf");
        replaceStringInFile("WORKGROUP", computerGroupEdit->text(), "/mnt/nioneniOS/etc/samba/smb.conf");
    }
    if (sambaCheckBox->isChecked()) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K01smbd /mnt/nioneniOS/etc/rc5.d/S06smbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K01smbd /mnt/nioneniOS/etc/rc4.d/S06smbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K01smbd /mnt/nioneniOS/etc/rc3.d/S06smbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K01smbd /mnt/nioneniOS/etc/rc2.d/S06smbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K01samba-ad-dc /mnt/nioneniOS/etc/rc5.d/S01samba-ad-dc >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K01samba-ad-dc /mnt/nioneniOS/etc/rc4.d/S01samba-ad-dc >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K01samba-ad-dc /mnt/nioneniOS/etc/rc3.d/S01samba-ad-dc >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K01samba-ad-dc /mnt/nioneniOS/etc/rc2.d/S01samba-ad-dc >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K01nmbd /mnt/nioneniOS/etc/rc5.d/S01nmbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K01nmbd /mnt/nioneniOS/etc/rc4.d/S01nmbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K01nmbd /mnt/nioneniOS/etc/rc3.d/S01nmbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K01nmbd /mnt/nioneniOS/etc/rc2.d/S01nmbd >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S06smbd /mnt/nioneniOS/etc/rc5.d/K01smbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S06smbd /mnt/nioneniOS/etc/rc4.d/K01smbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S06smbd /mnt/nioneniOS/etc/rc3.d/K01smbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S06smbd /mnt/nioneniOS/etc/rc2.d/K01smbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S01samba-ad-dc /mnt/nioneniOS/etc/rc5.d/K01samba-ad-dc >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S01samba-ad-dc /mnt/nioneniOS/etc/rc4.d/K01samba-ad-dc >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S01samba-ad-dc /mnt/nioneniOS/etc/rc3.d/K01samba-ad-dc >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S01samba-ad-dc /mnt/nioneniOS/etc/rc2.d/K01samba-ad-dc >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S01nmbd /mnt/nioneniOS/etc/rc5.d/K01nmbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S01nmbd /mnt/nioneniOS/etc/rc4.d/K01nmbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S01nmbd /mnt/nioneniOS/etc/rc3.d/K01nmbd >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S01nmbd /mnt/nioneniOS/etc/rc2.d/K01nmbd >/dev/null 2>&1");
    }

    replaceStringInFile("nioneniOS1", computerNameEdit->text(), "/mnt/nioneniOS/etc/hosts");

    QString cmd = QString("echo \"%1\" | cat > /mnt/nioneniOS/etc/hostname").arg(computerNameEdit->text());
    system(cmd.toUtf8());
    cmd = QString("echo \"%1\" | cat > /mnt/nioneniOS/etc/mailname").arg(computerNameEdit->text());
    system(cmd.toUtf8());
    cmd = QString("sed -i 's/.*send host-name.*/send host-name \"%1\";/g' /mnt/nioneniOS/etc/dhcp/dhclient.conf").arg(computerNameEdit->text());
    system(cmd.toUtf8());
    cmd = QString("echo \"%1\" | cat > /mnt/nioneniOS/etc/defaultdomain").arg(computerDomainEdit->text());
    system(cmd.toUtf8());
...
void MInstall::setLocale()
{
    ...
    //keyboard
    QString cmd = QString("chroot /mnt/nioneniOS /usr/sbin/install-keymap \"%1\"").arg(kb);
    system(cmd.toUtf8());
    if (kb == "uk") {
        kb = "gb";
    }
    if (kb == "us") {
        cmd = QString("sed -i 's/.*us/XKBLAYOUT=\"%1/g' /mnt/nioneniOS/etc/default/keyboard").arg(kb);
    } else if  (kb == "gb") {
        cmd = QString("sed -i 's/.*us/XKBLAYOUT=\"%1/g' /mnt/nioneniOS/etc/default/keyboard").arg(kb);
    } else {
       cmd = QString("sed -i 's/.*us/XKBLAYOUT=\"%1,us/g' /mnt/nioneniOS/etc/default/keyboard").arg(kb);
    }
    system(cmd.toUtf8());

    //locale
    cmd = QString("chroot /mnt/nioneniOS /usr/sbin/update-locale \"LANG=%1\"").arg(localeCombo->currentText());
    system(cmd.toUtf8());
    ...
    cmd = QString("ln -nfs /usr/share/zoneinfo/%1 /mnt/nioneniOS/etc/localtime").arg(timezoneCombo->currentText());
    system(cmd.toUtf8());
    ...
    cmd = QString("echo %1 > /mnt/nioneniOS/etc/timezone").arg(timezoneCombo->currentText());
    system(cmd.toUtf8());
    cmd = QString("echo %1 > /etc/timezone").arg(timezoneCombo->currentText());
    system(cmd.toUtf8());

    // timezone
    system("cp -f /etc/default/rcS /mnt/nioneniOS/etc/default");
    ...
    runCmd(QString("mount %1 /mnt/nioneniOS -o %2").arg(rootdev).arg(mntops));
    if (homedev != "/dev/root" && homedev != rootdev) {
        runCmd(QString("mount %1 /mnt/nioneniOS/home").arg(homedev));
    }
    system("cp -f /etc/adjtime /mnt/nioneniOS/etc/");

    // Set clock format - for icewm,fluxbox,jwm
    if (radio12h->isChecked()) {
        system("sed -i 's/%H:%M/%l:%M/g' /mnt/nioneniOS/etc/skel/.icewm/preferences");
        system("sed -i 's/%k:%M/%l:%M/g' /mnt/nioneniOS/etc/skel/.fluxbox/init");
        system("sed -i 's/%k:%M/%l:%M/g' /mnt/nioneniOS/etc/skel/.jwm/tray");
    } else {
        system("sed -i 's/%H:%M/%H:%M/g' /mnt/nioneniOS/etc/skel/.icewm/preferences");
        system("sed -i 's/%k:%M/%k:%M/g' /mnt/nioneniOS/etc/skel/.fluxbox/init");
        system("sed -i 's/%k:%M/%k:%M/g' /mnt/nioneniOS/etc/skel/.jwm/tray");
    }

    // localize repos
    runCmd("chroot /mnt/nioneniOS localize-repo default");
}
...
void MInstall::setServices()
{
    ...
    if (cpufreqItem != NULL && cpufreqItem->checkState(0) == Qt::Checked) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K96cpufrequtils /mnt/nioneniOS/etc/rc5.d/S04cpufrequtils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K96cpufrequtils /mnt/nioneniOS/etc/rc4.d/S04cpufrequtils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K96cpufrequtils /mnt/nioneniOS/etc/rc3.d/S04cpufrequtils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K96cpufrequtils /mnt/nioneniOS/etc/rc2.d/S04cpufrequtils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K97loadcpufreq /mnt/nioneniOS/etc/rc5.d/S03loadcpufreq >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K97loadcpufreq /mnt/nioneniOS/etc/rc4.d/S03loadcpufreq >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K97loadcpufreq /mnt/nioneniOS/etc/rc3.d/S03loadcpufreq >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K97loadcpufreq /mnt/nioneniOS/etc/rc2.d/S03loadcpufreq >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S04cpufrequtils /mnt/nioneniOS/etc/rc5.d/K96cpufrequtils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S04cpufrequtils /mnt/nioneniOS/etc/rc4.d/K96cpufrequtils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S04cpufrequtils /mnt/nioneniOS/etc/rc3.d/K96cpufrequtils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S04cpufrequtils /mnt/nioneniOS/etc/rc2.d/K96cpufrequtils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S03loadcpufreq /mnt/nioneniOS/etc/rc5.d/K97loadcpufreq >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S03loadcpufreq /mnt/nioneniOS/etc/rc4.d/K97loadcpufreq >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S03loadcpufreq /mnt/nioneniOS/etc/rc3.d/K97loadcpufreq >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S03loadcpufreq /mnt/nioneniOS/etc/rc2.d/K97loadcpufreq >/dev/null 2>&1");
    }

    if (wicdItem != NULL && wicdItem->checkState(0) == Qt::Checked) {
    system("mv -f /mnt/nioneniOS/etc/rc5.d/K01wicd /mnt/nioneniOS/etc/rc5.d/S04wicd >/dev/null 2>&1");
    system("mv -f /mnt/nioneniOS/etc/rc4.d/K01wicd /mnt/nioneniOS/etc/rc4.d/S04wicd >/dev/null 2>&1");
    system("mv -f /mnt/nioneniOS/etc/rc3.d/K01wicd /mnt/nioneniOS/etc/rc3.d/S04wicd >/dev/null 2>&1");
    system("mv -f /mnt/nioneniOS/etc/rc2.d/K01wicd /mnt/nioneniOS/etc/rc2.d/S04wicd >/dev/null 2>&1");
  } else {
    system("mv -f /mnt/nioneniOS/etc/rc5.d/S04wicd /mnt/nioneniOS/etc/rc5.d/K01wicd >/dev/null 2>&1");
    system("mv -f /mnt/nioneniOS/etc/rc4.d/S04wicd /mnt/nioneniOS/etc/rc4.d/K01wicd >/dev/null 2>&1");
    system("mv -f /mnt/nioneniOS/etc/rc3.d/S04wicd /mnt/nioneniOS/etc/rc3.d/K01wicd >/dev/null 2>&1");
    system("mv -f /mnt/nioneniOS/etc/rc2.d/S04wicd /mnt/nioneniOS/etc/rc2.d/K01wicd >/dev/null 2>&1");
  }

    if (bluetoothItem != NULL && bluetoothItem->checkState(0) == Qt::Checked) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K01bluetooth /mnt/nioneniOS/etc/rc5.d/S04bluetooth >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K01bluetooth /mnt/nioneniOS/etc/rc4.d/S04bluetooth >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K01bluetooth /mnt/nioneniOS/etc/rc3.d/S04bluetooth >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K01bluetooth /mnt/nioneniOS/etc/rc2.d/S04bluetooth >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S04bluetooth /mnt/nioneniOS/etc/rc5.d/K01bluetooth >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S04bluetooth /mnt/nioneniOS/etc/rc4.d/K01bluetooth >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S04bluetooth /mnt/nioneniOS/etc/rc3.d/K01bluetooth>/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S04bluetooth /mnt/nioneniOS/etc/rc2.d/K01bluetooth>/dev/null 2>&1");
    }

    if (sanedItem != NULL && sanedItem->checkState(0) == Qt::Checked) {
    	system("mv -f /mnt/nioneniOS/etc/rc5.d/K01saned /mnt/nioneniOS/etc/rc5.d/S05saned >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc4.d/K01saned /mnt/nioneniOS/etc/rc4.d/S05saned >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc3.d/K01saned /mnt/nioneniOS/etc/rc3.d/S05saned >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc2.d/K01saned /mnt/nioneniOS/etc/rc2.d/S05saned >/dev/null 2>&1");
    } else {
    	system("mv -f /mnt/nioneniOS/etc/rc5.d/S05saned /mnt/nioneniOS/etc/rc5.d/K01saned >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc4.d/S05saned /mnt/nioneniOS/etc/rc4.d/K01saned >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc3.d/S05saned /mnt/nioneniOS/etc/rc3.d/K01saned >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc2.d/S05saned /mnt/nioneniOS/etc/rc2.d/K01saned >/dev/null 2>&1");
    }

    if (sshItem != NULL && sshItem->checkState(0) == Qt::Checked) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K97ssh /mnt/nioneniOS/etc/rc5.d/S03ssh >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K97ssh /mnt/nioneniOS/etc/rc4.d/S03ssh >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K97ssh /mnt/nioneniOS/etc/rc3.d/S03ssh >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K97ssh /mnt/nioneniOS/etc/rc2.d/S03ssh >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S03ssh /mnt/nioneniOS/etc/rc5.d/K97ssh >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S03ssh /mnt/nioneniOS/etc/rc4.d/K97sh >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S03ssh /mnt/nioneniOS/etc/rc3.d/K97ssh >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S03ssh /mnt/nioneniOS/etc/rc2.d/K97ssh >/dev/null 2>&1");
    }

    if (avahiItem != NULL && avahiItem->checkState(0) == Qt::Checked) {
    	system("mv -f /mnt/nioneniOS/etc/rc5.d/K01avahi-daemon /mnt/nioneniOS/etc/rc5.d/S04avahi-daemon >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc4.d/K01avahi-daemon /mnt/nioneniOS/etc/rc4.d/S04avahi-daemon >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc3.d/K01avahi-daemon /mnt/nioneniOS/etc/rc3.d/S04avahi-daemon >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc2.d/K01avahi-daemon /mnt/nioneniOS/etc/rc2.d/S04avahi-daemon >/dev/null 2>&1");
    } else {
    	system("mv -f /mnt/nioneniOS/etc/rc5.d/S04avahi-daemon /mnt/nioneniOS/etc/rc5.d/K01avahi-daemon >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc4.d/S04avahi-daemon /mnt/nioneniOS/etc/rc4.d/K01avahi-daemon >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc3.d/S04avahi-daemon /mnt/nioneniOS/etc/rc3.d/K01avahi-daemon >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc2.d/S04avahi-daemon /mnt/nioneniOS/etc/rc2.d/K01avahi-daemon >/dev/null 2>&1");
    }
  
    if (acpidItem != NULL && acpidItem->checkState(0) == Qt::Checked) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K97acpid /mnt/nioneniOS/etc/rc5.d/S03acpid >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K97acpid /mnt/nioneniOS/etc/rc4.d/S03acpid >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K97acpid /mnt/nioneniOS/etc/rc3.d/S03acpid >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K97acpid /mnt/nioneniOS/etc/rc2.d/S03acpid >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S03acpid /mnt/nioneniOS/etc/rc5.d/K97acpid >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S03acpid /mnt/nioneniOS/etc/rc4.d/K97acpid >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S03acpid /mnt/nioneniOS/etc/rc3.d/K97acpid >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S03acpid /mnt/nioneniOS/etc/rc2.d/K97acpid >/dev/null 2>&1");
    }

    if (acpisupportItem != NULL && acpisupportItem->checkState(0) == Qt::Checked) {
    	system("mv -f /mnt/nioneniOS/etc/rc5.d/K01acpi-support /mnt/nioneniOS/etc/rc5.d/S03acpi-support >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc4.d/K01acpi-support /mnt/nioneniOS/etc/rc4.d/S03acpi-support >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc3.d/K01acpi-support /mnt/nioneniOS/etc/rc3.d/S03acpi-support >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc2.d/K01acpi-support /mnt/nioneniOS/etc/rc2.d/S03acpi-support >/dev/null 2>&1");
    } else {
    	system("mv -f /mnt/nioneniOS/etc/rc5.d/S03acpi-support /mnt/nioneniOS/etc/rc5.d/K01acpi-support >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc4.d/S03acpi-support /mnt/nioneniOS/etc/rc4.d/K01acpi-support >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc3.d/S03acpi-support /mnt/nioneniOS/etc/rc3.d/K01acpi-support >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc2.d/S03acpi-support /mnt/nioneniOS/etc/rc2.d/K01acpi-support >/dev/null 2>&1");
    }

    if (smartmontoolsItem != NULL && smartmontoolsItem->checkState(0) == Qt::Checked) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K01smartmontools /mnt/nioneniOS/etc/rc5.d/S03smartmontools >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K01smartmontools /mnt/nioneniOS/etc/rc4.d/S03smartmontools >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K01smartmontools /mnt/nioneniOS/etc/rc3.d/S03smartmontools >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K01smartmontools /mnt/nioneniOS/etc/rc2.d/S03smartmontools >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S03smartmontools /mnt/nioneniOS/etc/rc5.d/K01smartmontools >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S03smartmontools /mnt/nioneniOS/etc/rc4.d/K01smartmontools >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S03smartmontools /mnt/nioneniOS/etc/rc3.d/K01smartmontools >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S03smartmontools /mnt/nioneniOS/etc/rc2.d/K01smartmontools >/dev/null 2>&1");
    }

    if (rsyncItem != NULL && rsyncItem->checkState(0) == Qt::Checked) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K97rsync /mnt/nioneniOS/etc/rc5.d/S03rsync >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K97rsync /mnt/nioneniOS/etc/rc4.d/S03rsync >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K97rsync /mnt/nioneniOS/etc/rc3.d/S03rsync >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K97rsync /mnt/nioneniOS/etc/rc2.d/S03rsync >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S03rsync /mnt/nioneniOS/etc/rc5.d/K97rsync >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S03rsync /mnt/nioneniOS/etc/rc4.d/K97rsync >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S03rsync /mnt/nioneniOS/etc/rc3.d/K97rsync >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S03rsync /mnt/nioneniOS/etc/rc2.d/K97rsync >/dev/null 2>&1");
    }

    if (cupsItem != NULL && cupsItem->checkState(0) == Qt::Checked) {
    	system("mv -f /mnt/nioneniOS/etc/rc5.d/K01cups /mnt/nioneniOS/etc/rc5.d/S05cups >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc4.d/K01cups /mnt/nioneniOS/etc/rc4.d/S05cups >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc3.d/K01cups /mnt/nioneniOS/etc/rc3.d/S05cups >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc2.d/K01cups /mnt/nioneniOS/etc/rc2.d/S05cups >/dev/null 2>&1");
    } else {
    	system("mv -f /mnt/nioneniOS/etc/rc5.d/S05cups /mnt/nioneniOS/etc/rc5.d/K01cups >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc4.d/S05cups /mnt/nioneniOS/etc/rc4.d/K01cups >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc3.d/S05cups /mnt/nioneniOS/etc/rc3.d/K01cups >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc2.d/S05cups /mnt/nioneniOS/etc/rc2.d/K01cups >/dev/null 2>&1");
    }

    if (uuiddItem != NULL && uuiddItem->checkState(0) == Qt::Checked) {
    	system("mv -f /mnt/nioneniOS/etc/rc5.d/K01uuidd /mnt/nioneniOS/etc/rc5.d/S01uuidd >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc4.d/K01uuidd /mnt/nioneniOS/etc/rc4.d/S01uuidd >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc3.d/K01uuidd /mnt/nioneniOS/etc/rc3.d/S01uuidd >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc2.d/K01uuidd /mnt/nioneniOS/etc/rc2.d/S01uuidd >/dev/null 2>&1");
    } else {
    	system("mv -f /mnt/nioneniOS/etc/rc5.d/S01uuidd /mnt/nioneniOS/etc/rc5.d/K01uuidd >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc4.d/S01uuidd /mnt/nioneniOS/etc/rc4.d/K01uuidd >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc3.d/S01uuidd /mnt/nioneniOS/etc/rc3.d/K01uuidd >/dev/null 2>&1");
    	system("mv -f /mnt/nioneniOS/etc/rc2.d/S01uuidd /mnt/nioneniOS/etc/rc2.d/K01uuidd >/dev/null 2>&1");
    }

    if (dbusItem != NULL && dbusItem->checkState(0) == Qt::Checked) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K97dbus /mnt/nioneniOS/etc/rc5.d/S03dbus  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K97dbus /mnt/nioneniOS/etc/rc4.d/S03dbus  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K97dbus /mnt/nioneniOS/etc/rc3.d/S03dbus  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K97dbus /mnt/nioneniOS/etc/rc2.d/S03dbus  >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S03dbus  /mnt/nioneniOS/etc/rc5.d/K97dbus >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S03dbus  /mnt/nioneniOS/etc/rc4.d/K97dbus >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S03dbus  /mnt/nioneniOS/etc/rc3.d/K97dbus >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S03dbus  /mnt/nioneniOS/etc/rc2.d/K97dbus >/dev/null 2>&1");
    }

    if (cronItem != NULL && cronItem->checkState(0) == Qt::Checked) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K97cron /mnt/nioneniOS/etc/rc5.d/S03cron  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K97cron /mnt/nioneniOS/etc/rc4.d/S03cron  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K97cron /mnt/nioneniOS/etc/rc3.d/S03cron  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K97cron /mnt/nioneniOS/etc/rc2.d/S03cron  >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S03cron  /mnt/nioneniOS/etc/rc5.d/K97cron >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S03cron  /mnt/nioneniOS/etc/rc4.d/K97cron >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S03cron  /mnt/nioneniOS/etc/rc3.d/K97cron >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S03cron  /mnt/nioneniOS/etc/rc2.d/K97cron >/dev/null 2>&1");
    }

    if (gpmItem != NULL && gpmItem->checkState(0) == Qt::Checked) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K01gpm /mnt/nioneniOS/etc/rc5.d/S03gpm  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K01gpm /mnt/nioneniOS/etc/rc4.d/S03gpm  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K01gpm /mnt/nioneniOS/etc/rc3.d/S03gpm  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K01gpm /mnt/nioneniOS/etc/rc2.d/S03gpm  >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S03gpm  /mnt/nioneniOS/etc/rc5.d/K01gpm >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S03gpm  /mnt/nioneniOS/etc/rc4.d/K01gpm >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S03gpm  /mnt/nioneniOS/etc/rc3.d/K01gpm >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S03gpm  /mnt/nioneniOS/etc/rc2.d/K01gpm >/dev/null 2>&1");
    }

    if (sudoItem != NULL && sudoItem->checkState(0) == Qt::Checked) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/K82sudo /mnt/nioneniOS/etc/rc5.d/S01sudo  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/K82sudo /mnt/nioneniOS/etc/rc4.d/S01sudo  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/K82sudo /mnt/nioneniOS/etc/rc3.d/S01sudo  >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/K82sudo /mnt/nioneniOS/etc/rc2.d/S01sudo  >/dev/null 2>&1");
    } else {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S01sudo  /mnt/nioneniOS/etc/rc5.d/K82sudo >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S01sudo  /mnt/nioneniOS/etc/rc4.d/K82sudo >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S01sudo  /mnt/nioneniOS/etc/rc3.d/K82sudo >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S01sudo  /mnt/antiX/etc/rc2.d/K82sudo >/dev/null 2>&1");
    }

    if (!isInsideVB()) {
        system("mv -f /mnt/nioneniOS/etc/rc5.d/S02virtualbox-guest-utils /mnt/nioneniOS/etc/rc5.d/K01virtualbox-guest-utils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc4.d/S02virtualbox-guest-utils /mnt/nioneniOS/etc/rc4.d/K01virtualbox-guest-utils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc3.d/S02virtualbox-guest-utils /mnt/nioneniOS/etc/rc3.d/K01virtualbox-guest-utils >/dev/null 2>&1");
        system("mv -f /mnt/nioneniOS/etc/rc2.d/S02virtualbox-guest-utils /mnt/nioneniOS/etc/rc2.d/K01virtualbox-guest-utils >/dev/null 2>&1");
    }
...
void MInstall::stopInstall()
{
    ...
    if (curr == 3) {
        ...
        int ans = QMessageBox::information(0, QString::null,
                                           tr("nioneniOS Linux installation and configuration is complete.\n"
                                              "To use the new installation, reboot without the installation media.\n\n"
                                              "Do you want to reboot now?"),
                                           tr("Yes"), tr("No"));
        if (ans == 0) {
            system("/bin/rm -rf /mnt/nioneniOS/mnt/nioneniOS");
            system("/bin/umount -l /mnt/nioneniOS/home >/dev/null 2>&1");
            system("/bin/umount -l /mnt/nioneniOS >/dev/null 2>&1");
...
int MInstall::showPage(int curr, int next)
{
    if (next == 1 && curr == 0) {
        ...
        if (system("ls /home | grep -v lost+found | grep -v demo | grep -v snapshot | grep -q [a-zA-Z0-9]") == 0 || system("test -d /live/linux/home/demo") == 0) {
            setCursor(QCursor(Qt::WaitCursor));
            QString cmd = "rsync -a /home/ /mnt/nioneniOS/home/ --exclude '.cache' --exclude '.gvfs' --exclude '.dbus' --exclude '.Xauthority' --exclude '.ICEauthority'";
            system(cmd.toUtf8());
            setCursor(QCursor(Qt::ArrowCursor));
            next +=1;
        }
...
void MInstall::pageDisplayed(int next)
{
    ...
    case 1:
        ...
        ((MMain *)mmn)->setHelpText(tr("<p><b>General Instructions</b><br/>BEFORE PROCEEDING, CLOSE ALL OTHER APPLICATIONS.</p>"
                                       ...
                                       "<p>nioneniOS Linux requires about 3.5 GB of space. 5 GB or more is preferred. "
                                       "You can use the entire disk or you can put nioneniOS Linux on existing partitions. </p>"
                                       "<p>If you are running Mac OS or Windows OS (from Vista onwards), you may have to use that system's software to set up partitions and boot manager before installing nioneniOS Linux.</p>"
    ...
    case 2:
        ...
        ((MMain *)mmn)->setHelpText(tr("<p><b>Limitations</b><br/>Remember, this software is provided AS-IS with no warranty what-so-ever. "
                                       ...
                                       "<p><b>Choose Partitions</b><br/>nioneniOS Linux requires a root partition. The swap partition is optional but highly recommended. If you want to use the Suspend-to-Disk feature of nioneniOS Linux, you will need a swap partition that is larger than your physical memory size.</p>"
                                       ...
                                       "<p><b>Preferred Filesystem Type</b><br/>For nioneniOS Linux, you may choose to format the partitions as ext2, ext3, ext4, jfs, xfs, btrfs or reiser. </p>"
    ...
    case 3:
        ...
        tipsEdit->setText(tr("<p><b>Special Thanks</b><br/>Thanks to everyone who has chosen to support antiX Linux with their time, money, suggestions, work, praise, ideas, promotion, and/or encouragement.</p>"
                             "<p>Without you there would be no nioneniOS Linux.</p>"
                             "<p>nioneniOS Communities</p>"));
        ((MMain *)mmn)->setHelpText(tr("<p><b>Installation in Progress</b><br/>"
                                       "nioneniOS Linux is installing.  For a fresh install, this will probably take 3-20 minutes, depending on the speed of your system and the size of any partitions you are reformatting.</p>"
    ...
    case 4:
        ...
        setCursor(QCursor(Qt::ArrowCursor));
        ((MMain *)mmn)->setHelpText(tr("<p><b>Select Boot Method</b><br/>nioneniOS Linux uses the GRUB bootloader to boot nioneniOS Linux and MS-Windows. "
    ...
    case 9:
        setCursor(QCursor(Qt::ArrowCursor));
        ((MMain *)mmn)->setHelpText(tr("<p><b>Congratulations!</b><br/>You have completed the installation of ") + " nioneniOS Linux." + tr("</p>"
                                                                                                                                     "<p><b>Finding Applications</b><br/>There are hundreds of excellent applications installed with nioneniOS Linux. "
                                                                                                                                     ...
                                                                                                                                     "<p>In addition nioneniOS Linux includes many standard Linux applications that are run only from the commandline and therefore do not show up in Menu.</p>"));
        nextButton->setEnabled(true);
        backButton->setEnabled(false);
        break;
    ...
    default:
        // case 0 or any other
        ((MMain *)mmn)->setHelpText(tr("<p><b>Enjoy using nioneniOS Linux!</b></p>"));
        break;
    }
}
...
bool MInstall::close()
{
    if (proc->state() != QProcess::NotRunning) {
        int ans = QMessageBox::warning(0, QString::null,
                                       tr("nioneniOS Linux is installing, are you \nsure you want to Close now?"),
                                       tr("Yes"), tr("No"));
        if (ans != 0) {
            return false;
        } else {
...
void MInstall::delDone(int, QProcess::ExitStatus exitStatus)
{
    if (exitStatus == QProcess::NormalExit) {
        ...
        nextButton->setEnabled(true);
        unmountGoBack(tr("Failed to delete old nioneniOS Linux on destination.\nReturning to Step 1."));
    }
}
...
void MInstall::copyDone(int, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitStatus == QProcess::NormalExit) {
        ...
        chmod("/mnt/nioneniOS/var/tmp",01777);
        system("cd /mnt/nioneniOS && ln -s var/tmp tmp");

        FILE *fp = fopen("/mnt/nioneniOS/etc/fstab", "w");
        if (fp != NULL) {
...
//            }
        ...
        //system("/bin/rm -rf /mnt/nioneniOS/etc/skel/Desktop");
        system("/usr/sbin/live-to-installed /mnt/nioneniOS");
        runCmd("chroot /mnt/nioneniOS desktop-menu --write-out-global");
        system("/bin/rm -rf /mnt/nioneniOS/home/demo");
        system("/bin/rm -rf /mnt/nioneniOS/media/sd*");
        system("/bin/rm -rf /mnt/nioneniOS/media/hd*");
    //system("/bin/mv -f /mnt/nioneniOS/etc/X11/xorg.conf /mnt/nioneniOS/etc/X11/xorg.conf.live >/dev/null 2>&1");
        ...
        nextButton->setEnabled(true);
        unmountGoBack(tr("Failed to write nioneniOS Linux to destination.\nReturning to Step 1."));
    }
}
...
void MInstall::copyTime()
{
    ...
    QString val = getCmdValue("df /mnt/nioneniOS", rootdev, " ", "/");
    QRegExp sep("\\s+");
    ...
    case 1:
        tipsEdit->setText(tr("<p><b>Getting Help</b><br/>"
                             "Basic information about nioneniOS Linux is at https://antixlinux.com/. "
                             "Get support via https://t.me/nioneni</p>"
    ...
    case 15:
        tipsEdit->setText(tr("<p><b>Repairing Your Installation</b><br/>"
                             "If nioneniOS Linux stops working from the hard drive, sometimes it's possible to fix the problem by booting from CD and running one of the utilities in System Configuration or by using one of the regular Linux tools to repair the system.</p>"
                             "<p>You can also use your nioneniOS Linux CD to recover data from MS-Windows systems!</p>"));
        break;

    case 30:
        tipsEdit->setText(tr("<p><b>Support antiX Linux</b><br/>"
                             "nioneniOS Linux is supported by people like you. Some help others at the "
                             "Get support from https://t.me/nioneni "
    ...
    case 45:
        tipsEdit->setText(tr("<p><b>Adjusting Your Sound Mixer</b><br/>"
                             "nioneniOS Linux attempts to configure the sound mixer for you but sometimes it will be "
    ...
    case 60:
        tipsEdit->setText(tr("<p><b>Keep Your Copy of nioneniOS Linux up-to-date</b><br/>"
                             "For nioneniOS Linux information and updates please visit https://t.me/nioneni </p>"));
        break;
